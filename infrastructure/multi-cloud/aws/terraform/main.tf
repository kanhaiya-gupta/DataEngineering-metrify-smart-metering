...
resource "aws_security_group" "data_platform" {
  name_prefix = "${var.project_name}-data-platform"
  ...
  tags = {
    Name = "${var.project_name}-data-platform-sg"
  }
}
...
resource "aws_s3_bucket" "data_warehouse" {
  bucket = "${var.project_name}-data-warehouse-${random_string.bucket_suffix.result}"
...
resource "aws_cloudwatch_log_group" "data_platform" {
  name              = "/aws/eks/${var.project_name}-eks/data-platform"
  retention_in_days = 30

  tags = {
    Name = "${var.project_name}-data-platform-logs"
  }
}

# Application Load Balancer
resource "aws_lb" "data_platform" {
...
output "s3_data_warehouse_bucket" {
  description = "Name of the S3 data warehouse bucket"
  value       = aws_s3_bucket.data_warehouse.bucket
}
...
output "alb_dns_name" {
  ...
  value       = aws_lb.data_platform.dns_name
}
